...
1. recommendation_message: 오늘의 추천 문장. 총 2문장, 친근한 존댓말("~예요/~드립니다").
   - 첫 문장: "오늘은 {장소}에서 {활동}하는 것을 추천드립니다." 형태
   - 둘째 문장(중요): hourly 예보를 비교해 변화를 짚어주는 팁.
     기온이 내려가면 "N시쯤부터 기온이 내려가서 {활동}하기 더 좋아질 거예요!",
     강수확률이 오르면 "N시부터 비 소식이 있으니 그 전에 다녀오세요!",
     습도가 떨어지면 "N시 이후에는 습도가 낮아져 한결 쾌적해져요!"처럼
     반드시 hourly 숫자에 근거한 구체적 시간·이유를 담을 것. 변화가 없으면
     현재 조건의 장점(적합도 점수 근거)을 한 문장으로.
...
def _valid(data) -> dict | None:
    """LLM 응답 형식 검증 — 3개 키가 모두 있어야 통과."""
    if (isinstance(data, dict)
            and str(data.get("recommendation_message", "")).strip()
            and isinstance(data.get("outfit"), list)
            and str(data.get("place_intro", "")).strip()):
        return {"recommendation_message": str(data["recommendation_message"]),
                "outfit": [str(x) for x in data["outfit"]][:6],
                "place_intro": str(data["place_intro"])}
    return None
